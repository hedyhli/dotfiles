...
Elpaca docs: https://github.com/progfolio/elpaca/blob/master/doc/manual.md
...
I've had trouble updating Magit first due to the =git-commit= dependency (fixed by switching to GitHub repo rather than Elpa), then the transient dependency (fixed by installing through GitHub), and finally a "No elisp files in magit" error, fixed by adding the =:files= entry to =:ensure=.

For future reference:
- https://github.com/magit/magit/discussions/5180
...
Note that =fennel-ls= integration for Emacs <= 29 requires extra Eglot configuration (see below).
...
I've never quite gotten it to work though, unfortunately.
